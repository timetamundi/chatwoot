...
class Channel::Telegram < ApplicationRecord
  ...
  def convert_markdown_to_telegram_html(text)
    ...
    # Convert markdown to HTML, compatible with both commonmarker APIs.
    html = if defined?(Commonmarker)
             Commonmarker.to_html(escaped_text)
           else
             CommonMarker.render_html(escaped_text, [:HARDBREAKS], [:strikethrough])
           end.strip
